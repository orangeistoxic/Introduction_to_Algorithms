...
void DFS1(vector<Point> &APs, vector<int> &stack, int loca, int &cnt)
{
    ...
    {
        if(APs[loca].pTo[i]->start == -1)
        {
            DFS1(APs, stack, APs[loca].pTo[i] - &APs[0], cnt);
        }
    }
...
void DFS2(vector<Point> &APs, int loca, int &CSCC, long long &totV)
{
    ...
    {
        if(APs[loca].pFrom[i]->isV == false)
        {
            DFS2(APs, APs[loca].pFrom[i] - &APs[0], CSCC, totV);
        }
    }
}
...
int main()
{
    ...
    for(int i = 0; i < m; i++)
    {
        int a, b;
        cin >> a >> b;
        APs[a-1].pTo.push_back(&APs[b-1]);
        APs[b-1].pFrom.push_back(&APs[a-1]);
    }
